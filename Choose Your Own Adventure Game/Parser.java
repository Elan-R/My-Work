...
public class Parser {
    ...
    public World parse() throws IOException, SyntaxError, GameError {
        ...
        if (chr == Character.MAX_VALUE) {
            ...
            throw new SyntaxError("Context was not defined with \"#Menu#\" or \"#Screen#\"");
        }
        while (true) {
            switch (find_context()) {
    ...
    private void build_menu() throws IOException, SyntaxError {
        ...
        List<String> postmessages = new ArrayList<>(0);
        char chr;
        print("Finding ID");
        ...
        if (chr == '*') {
            ...
            if (chr == '"') {
                ...
                do {
                    print("Finding message");
                    postmessage.setLength(0);
                    while ((chr = read_string()) != 65534) {
                        postmessage.append(chr);
                    }
                    print("Message: " + postmessage.toString());
                    postmessages.add(postmessage.toString());
                    while ((chr = read()) != '}' && chr != '"');
                } while(chr != '}');
            }
        }
        menus.put(id.toString().strip(), new Menu(id.toString().strip(), premessages, message.toString(), choices, pointers, postmessages));
        print("Created menu");
    }
}
